...
export default function Newsletter({ newsLetterEmail, setNewsLetterEmail, newsLetterSuccess, handleSubscribe }: Props) {
  return (
    <div id="widget_23905" className="newsletter mb-5">
      <div className="big-title">Newsletter</div>
      {newsLetterSuccess ? (
        <div className="newsletter-description text-emerald-800 bg-emerald-50 p-2.5 rounded border border-emerald-200 mt-3 dark:bg-emerald-950 dark:text-emerald-200">{newsLetterSuccess}</div>
      ) : (
        <form onSubmit={handleSubscribe} className="newsletter-content" style={{ display: 'contents' }} id="registerForm">
          <div className="newsletter-title" style={{ fontFamily: 'Newsreader, serif' }}>
            Stay ahead with Saudi Gazette
          </div>
